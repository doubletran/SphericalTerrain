...
void Circle(float xc, float yc, float r, int numsegs)
{
...
void generate_terrain() {
	...
	for (int r = 0; r < n; r++) {
		...
		for (int c = 0; c <= n; c++) {
			...
			glVertex3f(r*tile_size, matrix[r][c], c*tile_size);
			glVertex3f((r + 1) * tile_size, matrix[r + 1][c], c * tile_size);
		}
		glEnd();
	}
